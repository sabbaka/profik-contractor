...
export default function OpenJobsMapScreen() {
  ...
  if (error) {
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity onPress={refetch} style={styles.retryBtn}><Text>Retry</Text></TouchableOpacity>
      </View>
    );
  }
  ...
  if (Platform.OS === 'web') {
    return (
      <View style={styles.container}>
        ...
        {(jobs || []).map((j: any) => (
          <TouchableOpacity key={j.id} style={styles.card} onPress={() => navigation.navigate('JobDetails', { id: j.id })}>
    ...
    <Maps.MapView style={styles.map} initialRegion={region}>
      {coords.map((j: any) => (
        <Maps.Marker
          ...
          onCalloutPress={() => navigation.navigate('JobDetails', { id: j.id })}
        />
      ))}
    </Maps.MapView>
  );
